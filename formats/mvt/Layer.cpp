...
namespace mvt::layer
{
	...
	IconAnchor IconAnchorToEnum(std::string_view iconAnchor)
	{
		...
		if (iconAnchor == "left") return IconAnchor::Left;
		if (iconAnchor == "right") return IconAnchor::Right;
		if (iconAnchor == "top") return IconAnchor::Top;
		if (iconAnchor == "top-left") return IconAnchor::TopLeft;
		if (iconAnchor == "top-right") return IconAnchor::TopRight;
		if (iconAnchor == "bottom-left") return IconAnchor::BottomLeft;
	...
	{
		if (symbolPlacement == "point") return SymbolPlacement::Point;
		if (symbolPlacement == "line") return SymbolPlacement::Line;
		if (symbolPlacement == "line-center") return SymbolPlacement::LineCenter;

		return SymbolPlacement::Point;
	}

	TextAnchor TextAnchorToEnum(std::string_view textAnchor)
	{
		if (textAnchor == "center") return TextAnchor::Center;
		if (textAnchor == "left") return TextAnchor::Left;
		if (textAnchor == "right") return TextAnchor::Right;
		if (textAnchor == "top") return TextAnchor::Top;
		if (textAnchor == "bottom") return TextAnchor::Bottom;
		if (textAnchor == "top-left") return TextAnchor::TopLeft;
		if (textAnchor == "top-right") return TextAnchor::TopRight;
		if (textAnchor == "bottom-left") return TextAnchor::BottomLeft;
		if (textAnchor == "bottom-right") return TextAnchor::BottomRight;

		return TextAnchor::Center;
	}

	TextJustify TextJustifyToEnum(std::string_view textJustify)
	{
		if (textJustify == "auto") return TextJustify::Auto;
		if (textJustify == "left") return TextJustify::Left;
		if (textJustify == "center") return TextJustify::Center;
